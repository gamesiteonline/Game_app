...
import { 
  Search, Filter, Download, Star, ArrowLeft, 
  Gamepad2, Smartphone, Info, HardDrive, Play,
  ChevronLeft, ChevronRight
} from "lucide-react";
import { PSPGame, PSP_GAMES } from "../data/pspGames";
import { GameDetailsModal } from "./GameDetailsModal";

interface PPSSPPGamesScreenProps {
  onBack: () => void;
  key?: string;
}
...
export const PPSSPPGamesScreen = ({ onBack }: PPSSPPGamesScreenProps) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedGenre, setSelectedGenre] = useState("All");
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedGame, setSelectedGame] = useState<PSPGame | null>(null);

  const genres = useMemo(() => {
    const allGenres = PSP_GAMES.map(g => g.genre.split(" / ")[0]);
    return ["All", ...Array.from(new Set(allGenres))];
  }, []);

  const filteredGames = useMemo(() => {
    return PSP_GAMES.filter(game => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tighter flex items-center gap-3">
              <Smartphone className="text-blue-500" /> PPSSPP <span className="text-gray-500 font-light">LIBRARY</span>
            </h1>
            <p className="text-sm text-gray-500 uppercase tracking-widest">High-Speed CSO/ISO Downloads</p>
          </div>
        </div>
        ...
        <AnimatePresence mode="popLayout">
          {paginatedGames.map((game, index) => (
            <motion.div
              ...
              className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/30 transition-all cursor-pointer"
            >
              {/* Game Image or Placeholder */}
              <div className="aspect-[16/9] bg-gradient-to-br from-blue-900/20 to-black relative overflow-hidden">
                {game.image ? (
                  <img 
                    src={game.image} 
                    alt={game.name} 
                ...
                <div className="absolute bottom-3 left-3 flex items-center gap-2">
                  <span className="px-2 py-0.5 bg-blue-600 text-[10px] font-bold rounded uppercase">CSO</span>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    className="flex-1 bg-white text-black py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-blue-500 hover:text-white transition-all shadow-lg"
                  >
